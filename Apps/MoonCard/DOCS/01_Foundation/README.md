...
MoonCard/
├── app/
│   └── moon_card_gallery.html   … アプリ本体（HTML/CSS/JS、データも内包）
├── assets/
│   └── moon_card_images/        … 各夜の雰囲気画像（card_01.jpg 〜）
├── data/
│   └── （将来：30夜分データをJSON等で外出しする場合はここに）
└── DOCS/
    ├── 01_Foundation/
    │   ├── MANIFEST.md
    │   ├── README.md             … このファイル
...
> `data/` フォルダへの分離は将来構想として `FUTURE_IDEAS.md` に記載します。
>
> このDOCS構成は、愛泉の杜OS全体の `DOC_STANDARD.md`（OS直下）に定義された
> Documentation Standard v1.0 に準拠しています。MoonCardはその Reference Implementation（最初の実装例）です。
